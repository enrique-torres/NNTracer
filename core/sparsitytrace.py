...
class SparsityTracing():
	def __init__(self, model, output_path, start_save_at=0, save_every_ith=10, capture_maximum=10):
		...
		self._activations_output_folder = "activation_traces"
		...
		self._activations_output_folder = output_path + "/activity/activation_traces"
		#self._weights_output_folder = output_path + "/activity/weight_traces"
	...
	def set_sparsity_tracing_state(self, state: bool):
	...
	def _activation_hook_fn(self, module, inputs, outputs):
		...
		for activation in inputs:
			try:
				...
				accum_zeroes += num_zeroes
				accum_elements += total_num_elements
		...
		try:
			...
			with open(self._activations_output_folder + "/" + module_name + ".csv", 'a+') as csvfile:
				...
				if create_first_row_csv:
					csv_writer.writerow(["num_zeroes","num_total_elements","sparsity_percentage"])
				csv_writer.writerow([accum_zeroes, accum_elements, (accum_zeroes/accum_elements) * 100])
				print([accum_zeroes, accum_elements, (accum_zeroes/accum_elements) * 100])
		except Exception as e:
			print(e)
